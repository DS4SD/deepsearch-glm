...
namespace andromeda
{
  ...
  bool nlp_model<ENT, LINK>::apply(subject<PARAGRAPH>& subj)
  {
  ...
  bool nlp_model<ENT, LINK>::apply_regex(subject<PARAGRAPH>& subj)
  {    
    ...
    for(auto& expr:exprs)
      {
	    ...
	    subj.entities.emplace_back(//utils::to_hash(name), //subj.entities.size(),
